...
* integers. */
void setup_simp_request(struct simp_msg_struct *msg_xmt, port_t my_port;) 
{ 
	int i;
...
* header. */
void setup_simp_reply(msg_header_t *msg_xmt, struct simp_msg_struct *msg_rcv;) 
{
...
port_t LookFor(env_name_t name) 
{
	port_t result; 
	kern_return_t error;
	if ((error = env_get_port(environment_port, name, &result)) != KERN_SUCCESS) 
		{
...
port_t Register(env_name_t name)  
{
	port_t result; 
	kern_return_t error;
	...
	if ((error = env_set_port(environment_port, name, result)) != KERN_SUCCESS) 
		{
...
void child_routine(port_t my_port) ;
{
	msg_return_t ret;
	int i; 
	struct simp_msg_struct msg_xmt, msg_rcv;
	setup_simp_request(&msg_xmt, my_port);
	if ((ret = msg_rpc(&msg_xmt.h, MSG_OPTION_NONE, sizeof(msg_xmt), 0, 0)) != RPC_SUCCESS)
		{
...
void parent_routine(port_t my_port) 
{
	msg_return_t ret; 
	int i;
	int x; 
	...
	if ((ret = msg_receive(&msg_rcv.h, MSG_OPTION_NONE, 0)) != RCV_SUCCESS) 
		{
	...
	printf("PARENT: Data.."); 
	for (i = 0; i < MAXDATA; i++)
		printf("%d ", msg_rcv.inline_data[i]); printf("\n");
	...
	if (fret == 0) 
		{ /* child process */
			...
			{ /* parent process */ 
				...
				if ((err = env_del_port(environment_port, port_name)) != KERN_SUCCESS) 
					{
						mach_error("PARENT: env_del_port returned ", err);
						exit(1);
					}
				if ((err = port_deallocate(task_self(), my_port)) != KERN_SUCCESS) 
					{
